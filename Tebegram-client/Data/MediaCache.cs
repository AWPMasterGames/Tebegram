...
{
    /// <summary>
    /// Локальный кэш медиа: один раз открытые (или просто показанные в чате) фото и
    /// видео остаются на диске, и второй раз с сервера не качаются вообще.
    ///
    /// КАК ЭТО СДЕЛАНО В ОРИГИНАЛЬНОМ TELEGRAM (с чего срисовано):
    /// Telegram Desktop держит кэш в %APPDATA%\Telegram Desktop\tdata\user_data\ —
    /// там два хранилища: cache (мелочь: превью, стикеры, аватарки) и media_cache
    /// (тяжёлое: видео и документы, которые качаются потоком). Внутри — файлы с
    /// обезличенными именами, разложенные по подпапкам, плюс журнал (binlog) с
    /// метаданными; всё шифруется локальным ключом. У кэша есть потолок размера и
    /// срок хранения (Настройки → Продвинутые → Управление памятью), лишнее
    /// вытесняется по «давно не использовалось». На Android то же самое лежит в
    /// Android/media/org.telegram.messenger/Telegram/… (Telegram Images, Telegram
    /// Video), на iOS — в Library/Caches песочницы приложения.
    ///
    /// ЧТО ИЗ ЭТОГО ВЗЯТО СЮДА:
    /// — отдельная папка кэша в %LOCALAPPDATA%\Tebegram\cache (files — оригиналы,
    ///   preview — маленькие картинки для пузырей и кадры видео);
    /// — ключ = имя файла НА СЕРВЕРЕ, а не полный URL: адрес сервера меняется
    ///   (туннель каждый раз новый), а имя файла в /upload/ постоянно и уникально;
    /// — потолок размера и вытеснение самых старых записей;
    /// — два уровня: в списке сообщений показывается уменьшенное превью (быстро
    ///   декодируется, мало памяти), оригинал нужен только просмотрщику.
    ///
    /// Шифрования здесь нет намеренно: Telegram шифрует кэш локальным ключом от
    /// облачного пароля, у нас такого ключа нет — вводить его ради этого было бы
    /// отдельной большой темой.
    /// </summary>
