...
{
    /// <summary>
    /// Обработка исходящего звука звонка на клиенте Windows, PCM Int16, моно.
    ///
    /// В браузере подавлением шума и эха занимается движок getUserMedia, у NAudio
    /// такой обработки нет, поэтому она выполняется здесь. Порядок этапов:
    ///
    /// 1) фильтр высоких частот 120 Гц срезает низкочастотный гул от вибраций и
    ///    движения воздуха;
    /// 2) адаптивный шумовой гейт отслеживает уровень фона и открывается, когда
    ///    сигнал заметно превышает его; между словами фон приглушается;
    /// 3) мягкая нормализация подтягивает речь к целевой громкости. Усиление
    ///    применяется только при открытом гейте, поэтому шум не усиливается.
    ///
    /// Буфер обрабатывается на месте, состояние сохраняется между вызовами.
    /// </summary>
