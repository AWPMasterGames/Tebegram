...
app.Map("/Voice/ws", async context =>
{
        ...
        && joinedRoom.RoomMembers.Count >= 2)
    {
        ...
        // Звонок принят - гасим токен у обеих сторон.
        //
        // Пользователь может быть в сети сразу на нескольких устройствах, и окно
        // входящего вызова показывается на каждом. Отвечает он на одном, а
        // остальные окна следят именно за токеном (см. CheckIncomingStillAliveAsync)
        // и закрываются, когда тот исчезает. Без этой строки такое окно висело до
        // ручного закрытия, а закрытие обрывало уже идущий разговор.
        UsersData.ClearCallTokens(Token);
    }
